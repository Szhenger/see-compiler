...
WeightFoldingPass::run(sir::Block& block, utility::WeightBuffer& weights) {
    ...
    utility::Logger::info(std::format(
        "WeightFoldingPass: {} fused Conv+BN op(s) to fold", 
        fused_convs.size()
    ));
    ...
    utility::Logger::info(std::format(
        "WeightFoldingPass: folded {} Conv+BN weight pair(s)", 
        folded_count_
    ));
...
std::expected<void, CodegenError>
WeightFoldingPass::foldConv(sir::Operation* conv_op, utility::WeightBuffer& weights) {
    ...
    if (bn_bias_sp->size() != F || mean_span->size() != F || var_span->size() != F) {
        return std::unexpected(CodegenError{
            "weight_folding",
            std::format("BN parameter size mismatch — scale has size {}, but all four "
                        "tensors must match the output channel dimension (F={})", scale_span->size(), F)
        });
    }
    ...
    if (!filter_span) {
        return std::unexpected(CodegenError{
            "weight_folding",
            std::format("Conv filter '{}' not found in WeightBuffer", filter_id)
        });
    }

    // Filter layout assumed: [F, C/group, KH, KW]
    ...
    if (elems_per_out * F != filter_total) {
        return std::unexpected(CodegenError{
            "weight_folding",
            std::format("Filter element count {} is not divisible by output channels F={}", filter_total, F)
        });
    }
    ...
    if (conv_op->numOperands() >= 3) {
        ...
        if (bias_span) {
            if (bias_span->size() != F) {
                return std::unexpected(CodegenError{
                    "weight_folding",
                    std::format("Conv bias size {} does not match output channels F={}", bias_span->size(), F)
                });
            }
    ...
    // Mathematically, we are resolving the linear transformation at compile-time:
    // std_inv = 1 / sqrt(var + eps)
    // scale_factor = gamma * std_inv
    // W_folded = W * scale_factor
    // b_folded = (b - mean) * scale_factor + beta
    ...
    for (size_t f = 0; f < F; ++f) {
        ...
        // Scale every spatial/channel item tied to output channel f
        ...
        b_fused[f] = (bias_data[f] - (*mean_span)[f]) * scale_factor + (*bn_bias_sp)[f];
    }

    // --- 6. Write folded tensors back to WeightBuffer ---
    const std::string w_folded_key = std::format("{}__bn_folded", filter_id);
    const std::string b_folded_key = std::format("{}__bn_folded_bias", filter_id);
    ...
    utility::Logger::info(std::format(
        "WeightFoldingPass: folded '{}' -> '{}' [F={}, elems_per_channel={}]",
        filter_id, w_folded_key, F, elems_per_out
    ));
